	...
	if ( $blog_id > 0 ) {
		...
		if ( $force || $excerpt === '' ) {
			wp_update_post(
				array(
					'ID'           => $blog_id,
					'post_excerpt' => __( 'Accessible holiday tips, guest stories, what’s on around Whitstable, and practical updates for wheelchair users, carers and anyone planning a respite break.', 'restwell-retreats' ),
				)
			);
