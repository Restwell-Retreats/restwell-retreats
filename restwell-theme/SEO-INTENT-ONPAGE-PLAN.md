...
| `/pricing/`                                         | `template-pricing.php`                      | Conversion / commercial | Own accessible holiday pricing Whitstable; rates, funding, care guide.       | accessible holiday pricing whitstable                          |
...
| `/terms-and-conditions/` | `restwell terms`                                            | Booking confirmation, payments (deposit, balance one week before arrival, BACS and card), cancellation tiers, accessibility reliance at booking, exceptional circumstances, optional care introduction, equipment use and hoist, dogs with notice and risk assessment |
...
- **§4 Step F / P4 Step E (2026-07-27):** Pillar↔cluster internal linking centralised in `inc/internal-links.php` (Related guides on pillars, Part of + siblings on singles, home hub links, conversion reachability, orphan audit). Matrix column **E** set `x` for `/`, three pillars, `/enquire/`, and all 19 seeded cluster posts.
...
6. **H2:** Deposits, balance, and payment methods (Step A **restwell terms bookings payments**, **booking deposits cancellations accessible stay**; BACS and card; balance no later than one week before arrival unless agreed otherwise in writing, per theme default via `restwell_get_payment_timeline()`).
...
| `/pricing/` (`template-pricing.php`)                                       | 2026-07-27     | 1    | accessible holiday pricing whitstable                     | accessible bungalow rates whitstable, peak off-peak holiday pricing kent, NHS CHC respite holiday cost, Continuity of Care Services guide rates                                                                                                                                    | Accessible Holiday Pricing in Whitstable \| Restwell Retreats     | How pricing works for an accessible self-catering break at Restwell in Whitstable, with private, case-managed and NHS CHC funding routes and deposits explained. | **H1** Pricing for your accessible Whitstable break. **Subheading** Transparent accessible holiday pricing in Whitstable: the bungalow, optional care and every funding route explained. **H2** Our rates; What is included in the price; Access equipment is always included; How payment works; Three ways to fund your stay; Optional care while you stay; What else to budget for; Booking is built to be accessible too; Common questions about pricing; Ready to plan your accessible break? | Full 5-Q pricing FAQ (visible + FAQPage JSON-LD via `restwell_get_faq_items( 'pricing' )`). | Rates from `restwell_get_pricing()`; links to The Property, Accessibility, How It Works, Resources, /dates/ (parked → /enquire/), /enquire/. Primary CTA: Check dates and care availability. Care guide rates next review 1 September 2026. | Shipped 2026-07-27 (theme). Polish + signed copy 2026-07-27: hero subheading (primary keyphrase in first 100 words); interior-hero + resources TOC; rw-table; closing CTA. Confirm resolved: slings BYO; payment one week via `restwell_get_payment_timeline()` (+ terms migration); guest guide 15:00/11:00; /dates/ parked; care figures unchanged. Still on Ellie: publish /pricing/ + SEO meta (Theme Setup, permalink flush, logged-out check). |
