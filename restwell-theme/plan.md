...
- [x] **Editorial intent split** - One primary keyphrase per indexable page via `inc/seo-keyword-map.php` + lane locks on Home / Property / Accessibility / How It Works / Who It's For / Pricing (`accessible holiday pricing whitstable`). → Verify: `restwell_keyword_cannibalisation_audit()` reports zero keyphrase or H1 clashes.
